...
namespace Ext
{
	...
	void AgentUpdate(EvCombatData* evCbtData)
	{
		...
		if (evAgentUpdate.target)
		{
			...
			G::APIDefs->Events.Raise(EV_ARCDPS_TARGET_CHANGED, &evAgentUpdate);
		...
		if (!evAgentUpdate.added)
		{
			...
			if (s_Self.id != 0 && s_Self.id == evAgentUpdate.id)
			{
				...
				G::APIDefs->Events.Raise(EV_ARCDPS_SELF_LEAVE, &evAgentUpdate);
			}
			else
			{
					...
					[&](EvAgentUpdate const& member)
				{
					if (member.id == evAgentUpdate.id)
					{
						...
						G::APIDefs->Events.Raise(EV_ARCDPS_SQUAD_LEAVE, &evAgentUpdate);
		...
		if (!evAgentUpdate.Self)
		{
			...
			G::APIDefs->Events.Raise(EV_ARCDPS_SQUAD_JOIN, &evAgentUpdate);
...

		{
			...
			G::APIDefs->Events.Raise(EV_ARCDPS_SELF_JOIN, (void*)&s_Self);

			if (G::AccountName.empty())
			{
				G::AccountName = s_Self.account;
	...
	void OnSquadRequest(void* eventArgs)
	{
		...
		std::thread([]()
		{
			...
			{
				G::APIDefs->Events.Raise(EV_ARCDPS_SQUAD_JOIN, (void*)&member);
			}
		}).detach();
	}
	...
	void OnTargetRequest(void* eventArgs)
	{
		...
		std::thread([]()
		{
			G::APIDefs->Events.Raise(EV_ARCDPS_TARGET_CHANGED, (void*)&s_LastTarget);
		}).detach();
	}
}
